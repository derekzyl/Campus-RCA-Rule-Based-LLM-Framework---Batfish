...
|---|---:|---:|---:|---:|---:|
| hybrid | 1.0 | 0.747 | 0.0 | 0.8 | 62936.7 |
| rule_only | 1.0 | 0.78 | 0.0 | 1.0 | 3035.1 |
| llm_only | 0.9 | 0.76 | 0.0 | 0.45 | 17158.8 |
...
- `llm_only` / `student_acl_deny_mgt` [OK] pred=`acl_deny`@core_sw1 truth=`acl_deny`@core_sw1
...
- `llm_only` / `guest_wlan_acl_deny` [OK] pred=`acl_deny`@core_sw1 truth=`acl_deny`@core_sw1
...
- `llm_only` / `missing_ospf_students` [OK] pred=`missing_route`@dsw_b_student truth=`missing_route`@dsw_b_student
...
- `llm_only` / `core1_uplink_shutdown` [OK] pred=`interface_down`@core_sw1 truth=`interface_down`@core_sw1
...
- `llm_only` / `wrong_default_route_r1` [OK] pred=`wrong_static_route`@campus_r1 truth=`wrong_static_route`@campus_r1
...
- `llm_only` / `ospf_omit_academic` [OK] pred=`missing_route`@dsw_a_acad truth=`missing_route`@dsw_a_acad
...
- `llm_only` / `dmz_to_lan_leak_attempt` [OK] pred=`acl_deny`@fw1 truth=`acl_deny`@fw1
...
- `llm_only` / `fw1_inside_shutdown` [OK] pred=`interface_down`@fw1 truth=`interface_down`@fw1
...
- `llm_only` / `core2_student_uplink_down` [MISS] pred=`unknown`@core_sw2 truth=`interface_down`@core_sw2
...
- `llm_only` / `missing_ospf_dns_services` [OK] pred=`missing_route`@dsw_d_dc truth=`missing_route`@dsw_d_dc